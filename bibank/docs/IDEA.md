...
BiBank:
- Tiền của user ở trong hệ BiBank
- Không cần đối chiếu với ngân hàng khác cho 95% flow nội bộ
- Internal transfer = in-memory state change
👉 Không phải BiBank reconcile tốt
👉 Mà là BiBank loại bỏ nhu cầu reconcile
...
### 3. BiBank thiết kế theo event-sourcing từ ngày 1
Event là sự thật
...
### 5. BiBank Risk Engine chứ không chỉ ledger
...
### 6. AML: BiBank làm tốt vì kiểm soát FLOW, không chỉ DATA
...
BiBank là real-time financial OS
	- Không legacy core
	- Không COBOL
	- Không nightly batch
	- Không “đợi đối soát”
...
## ROADMAP KỸ THUẬT CỦA BiBank

Dưới đây là **roadmap map TRỰC TIẾP repo `ledger` của BiBank-class core**, theo đúng 4 khối bạn nêu:
...
Nếu không giữ 4 nguyên tắc này, không bao giờ chạm được BiBank-class.
...
👉 Điều này là **linh hồn kế toán BiBank**.
...
# 3. CORE 3 — Risk Engine (điểm BiBank vượt ngân hàng)
...
# 5. Tổng hợp kiến trúc (BiBank-class)
